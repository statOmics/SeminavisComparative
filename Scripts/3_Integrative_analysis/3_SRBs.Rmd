...
```{r}
minGenes <- rownames(res_tr_min)[res_tr_min$padjScreen == 1]
minGenes <- intersect(minGenes,rownames(fit))

# Test only genes withheld in the first stage (DE in MT-)
tr15 <- glmTreat(fitPlus[minGenes,], contrast = as.data.frame(L)$t15, lfc= log2(3))
tr1 <- glmTreat(fitPlus[minGenes,], contrast = as.data.frame(L)$t1, lfc= log2(3))
tr3 <- glmTreat(fitPlus[minGenes,], contrast = as.data.frame(L)$t3, lfc= log2(3))

# Aggregate p-values
allPMinToPlus <- data.frame(tr15$table$PValue, tr1$table$PValue, tr3$table$PValue)
colnames(allPMinToPlus) <- c("t15", "t1", "t3")
rownames(allPMinToPlus) <- rownames(tr15)

pScreenMinToPlus <- apply(allPMinToPlus, 1, function(x) sidak(x))
names(pScreenMinToPlus) <- rownames(tr15)
pConfirmationMinToPlus <- as.matrix(allPMinToPlus)

objMinToPlus <- stageR(pConfirmation = pConfirmationMinToPlus, pScreen = pScreenMinToPlus)
objMinToPlus <- stageWiseAdjustment(object = objMinToPlus, method = "holm", alpha = 0.05)

resMinToPlus <- as.data.frame(getResults(objMinToPlus))
colSums(resMinToPlus)
minToPlusGenes <- rownames(resMinToPlus)[resMinToPlus[,1]==1]
