...
# Equivalence testing for the SIP paper
...
A more elegant way to perform TOST would be to use the Wald test that Koen and Lieven have been developing, but since it is still being worked on we will use the LRT of EdgeR for the SIP paper.

For MT+, only the first 3 timepoints are checked if they are DE 


The model formula of the SIP RNA-seq experiment with 3 timepoints is: 
$$ log(Y_{i}) = \beta_{c0} + \beta_{c15}X_{c15} + \beta_{t15}X_{t15} + \beta_{c1}X_{c1} + \beta_{c1}X_{c1} + \beta_{c3}X_{c3} + \beta_{c3}X_{c3} $$
...
### Load data into DGElist object and then filter & normalizae
...
```{r}
# function that returns a data frame of equivalence p-values (unadjusted!) for the three contrasts at a given FC cutoff
...
log2FC <- log2FcMatrix_min[rownames(log2FcMatrix_min) %in% equiv3_LRT,]
...
### COMMENTED OUT FOR INTERSECTION ANALYSIS
...
logFC_ML <- logFcMatrix[rownames(logFcMatrix) %in% names,]
...
> As all "non-responsive" genes belong to the "filtered" (non expressed) class, we cannot plot their FC. Alternatively we could plot the filtered genes as having a log2FC of "0" (see Figure 4 of the paper)


```{r}
minGenes <- as.character(non_responsive$gene_ID)
plusGenes <- as.character(out$names)
interGenes <- intersect(minGenes, plusGenes)
write.table(interGenes, file="intersectionSRPGenes.txt", quote=FALSE, row.names = FALSE, col.names = FALSE)
```
